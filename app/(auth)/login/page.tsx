  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-16">
      ...
      <p className="mt-2 text-sm text-ink-muted">
        This area is for Relentless Lab School &amp; College staff only.
      </p>

      {notice && (
        <p
          role="status"
          className="mt-6 rounded-sm bg-green-mist px-3 py-2 text-sm text-green-ink"
        >
          {notice}
        </p>
      )}
      {errorNotice && (
        <p
          role="alert"
          className="mt-6 rounded-sm bg-danger-bg px-3 py-2 text-sm text-danger"
        >
          {errorNotice}
        </p>
      )}
