...
export async function generateMetadata({
  ...
  return {
    title: `Apply — ${label}`,
    description: `Apply for the ${label} programme at Relentless Lab School & College, Lakeview.`,
...
function ApplyShell({
  ...
  return (
    <Band tone="paper">
      {/* ONE centred reading column for every state on this route — the same fix
          already made on /programmes/[slug]. The Band's Container runs to
          1440px, so an uncentred max-w-* child sits flush-left against a wide
          empty right-hand side. Centring the shell rather than each state means
          a state added later inherits it instead of reintroducing the bug.
          Only the COLUMN is centred: text, labels and fields inside keep their
      ...
      <div className="mx-auto max-w-3xl">
        <Reveal>
          ...
          {/* max-w-3xl dropped here — the column above now caps the width, and
              two caps on the same axis is one to forget when either changes. */}
          <h1 className="mt-2 font-display text-h1 text-green-ink">{title}</h1>
        </Reveal>
        {children}
      </div>
    </Band>
  ...
  if (!form.is_published) {
    return (
      <ApplyShell eyebrow="Admissions" title={form.title}>
        <Reveal className="mx-auto mt-10 max-w-2xl rounded-lg border border-line bg-surface p-8 sm:p-10">
