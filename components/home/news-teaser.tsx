...
export async function NewsTeaser() {
  ...
  return (
    // brand, not forest: in the homepage order this is still the CONTRASTING
    // break in a run of light bands (Voice above is paper, Facilities below is
    // paper), but it is now a mid green rather than a third helping of the dark
    // forest already carried by the hero, the closing CTA, and the footer.
    // The cards below stay --surface with their own dark text (--green-ink at
    // 15.37:1, --ink-muted at 6.04:1 on white), so they are unaffected by the
    // band colour entirely.
    //
    // The header text WAS re-measured against the new, lighter ground rather
    // than carried over: --green-pale reads 9.66:1 on forest but only 4.22:1 on
    // --green-brand, which FAILS AA for normal text. The eyebrow and the link
    // therefore move to --paper (5.17:1). The h2 keeps `text-white` — 5.32:1
    // here — so the heading stays the brightest thing in the header.
    <Band tone="brand">
      <Reveal className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-eyebrow uppercase tracking-wide text-paper">
            Latest
          </p>
        ...
        <Link
          href="/news"
          className="text-small font-medium text-paper hover:underline"
        >
          View all news →
        </Link>
      </Reveal>
