...
export function ScrollableCardDialog<T>({
  ...
  /**
   * Set on a dark Band (e.g. tone="brand"). The header's default colours are
   * for LIGHT grounds and are not merely dim on a green band — measured on
   * --green-brand the eyebrow's --green-brand is 1.00:1 (the same colour as the
   * background, i.e. invisible), the --green-ink heading is 2.89:1 and the
   * --ink-muted helper line is 1.14:1. All three fail AA. The on-dark set is
   * measured on --green-brand: --paper 5.17:1, white 5.32:1, --green-mist
   * 4.69:1 — all pass, and the helper stays a step quieter than the eyebrow.
   * The CARDS are unaffected either way: they carry their own --surface.
   */
  onDark?: boolean;
}) {
  ...
  return (
    <>
      <Reveal>
        {/* The same opener as <SectionHeading> — accent rule, small caps, serif
            heading — inlined rather than composed, because this block also owns
            the conditional helper line below and the two must share one Reveal.
            Keep the two in step if either changes. */}
        <p
          className={`flex items-center gap-3 text-eyebrow uppercase ${
            onDark ? 'text-signature' : 'text-signature-ink'
          }`}
        >
        ...
        <h2
          className={`mt-5 max-w-3xl font-display text-h2 text-balance ${
            onDark ? 'text-cream' : 'text-ink-950'
          }`}
        >
          {heading}
        </h2>
        ...
        {items.length > MAX_CARDS_IN_VIEW && (
          <p
            className={`mt-3 text-small ${
              onDark ? 'text-green-mist' : 'text-ink-muted'
            }`}
          >
            {helperText}
          </p>
