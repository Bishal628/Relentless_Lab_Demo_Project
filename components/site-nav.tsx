...
const LOGO_ALT = 'Relentless Lab School & College';
...
export function SiteNav() {
  ...
  // ONE flag drives EVERYTHING below — background, links, and wordmark all read
  // `transparent`, so the bar can never be white-on-white: transparent bg always
  // pairs with the cream wordmark and white links, the solid cream bar always
  // pairs with the ink wordmark and dark links. Transparent only while over the
  // hero and at the top.
  ...
  return (
    ...
    >
      ...
      >
        {/* The wordmark (components/wordmark.tsx) — pure type, no image asset.
            Cream over the hero, ink on the solid bar, driven by the same
            `transparent` flag as the links. `nav-legible` adds the text shadow
            that keeps it readable over the light parts of the hero video. */}
        <Link
          href="/"
        ...
        >
          <Wordmark
            ...
            className={transparent ? 'nav-legible' : ''}
          />
        </Link>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          {/* Hamburger. Shown until `xl` (1280px), not `md`: the grouped labels
              ("Life at Relentless Lab", "News & media") are far wider than the old flat
      ...
      {menuOpen && (
        ...
        >
          ...
          >
            ...
            >
              {/* Always the dark tone: the sheet is a full-screen ink panel. */}
              <Wordmark tone="dark" />
            </Link>
            <button
              type="button"
